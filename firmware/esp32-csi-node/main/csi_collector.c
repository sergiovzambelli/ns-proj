...
static void wifi_csi_callback(void *ctx, wifi_csi_info_t *info)
{
    ...
    if (frame_len > 0) {
        ...
        if ((now - s_last_send_us) >= CSI_MIN_SEND_INTERVAL_US) {
            int ret;
            if (csi_crypto_enabled() && frame_len > 5) {
                /* Build ADR-018-enc encrypted frame:
                 *   [0xC5110003 LE (4B)] [NodeID (1B)] [Nonce (12B)]
                 *   [Ciphertext (frame_len-5 B)] [Tag (16B)]
                 *
                 * Static buffer: safe because CSI sends are rate-limited
                 * to one at a time (CSI_MIN_SEND_INTERVAL_US gate above). */
                static uint8_t s_enc_buf[CSI_MAX_FRAME_SIZE + CSI_CRYPTO_NONCE_LEN + CSI_CRYPTO_TAG_LEN];
                uint32_t enc_magic = CSI_ENC_MAGIC;
                memcpy(s_enc_buf, &enc_magic, 4);   /* new magic */
                s_enc_buf[4] = frame_buf[4];        /* node_id (plaintext / AAD) */

                size_t plain_len = frame_len - 5;
                uint8_t *nonce_out  = s_enc_buf + 5;
                uint8_t *cipher_out = s_enc_buf + 5 + CSI_CRYPTO_NONCE_LEN;
                uint8_t *tag_out    = cipher_out + plain_len;

                int enc_ret = csi_crypto_encrypt(
                    s_enc_buf, 5,             /* AAD: new magic + node_id */
                    frame_buf + 5, plain_len, /* plaintext: ADR-018 bytes 5..end */
                    nonce_out, cipher_out, tag_out);

                if (enc_ret == 0) {
                    size_t enc_len = 5 + CSI_CRYPTO_NONCE_LEN + plain_len + CSI_CRYPTO_TAG_LEN;
                    ret = stream_sender_send(s_enc_buf, enc_len);
                } else {
                    ret = -1;
                }
            } else {
                ret = stream_sender_send(frame_buf, frame_len);
            }
            if (ret > 0) {
                s_send_ok++;
                s_last_send_us = now;
