...
void nvs_config_load(nvs_config_t *cfg)
{
    ...
    /* AES-128-GCM key for encrypted UDP streaming (ADR-018-enc). */
    cfg->aes_key_valid = 0;
    memset(cfg->aes_key, 0, sizeof(cfg->aes_key));
    size_t aes_key_len = sizeof(cfg->aes_key);
    if (nvs_get_blob(handle, "aes_key", cfg->aes_key, &aes_key_len) == ESP_OK
        && aes_key_len == 16)
    {
        cfg->aes_key_valid = 1;
        ESP_LOGI(TAG, "NVS: aes_key loaded (%02x%02x...%02x%02x)",
                 cfg->aes_key[0], cfg->aes_key[1],
                 cfg->aes_key[14], cfg->aes_key[15]);
    } else {
        ESP_LOGW(TAG, "No aes_key in NVS — CSI frames will be sent unencrypted");
    }
