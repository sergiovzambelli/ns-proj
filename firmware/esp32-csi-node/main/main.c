...
void app_main(void)
{
    ...
    /* Initialize AES-128-GCM if a key was provisioned. */
    if (g_nvs_config.aes_key_valid) {
        if (csi_crypto_init(g_nvs_config.aes_key, 16) == 0) {
            ESP_LOGI(TAG, "AES-GCM encryption enabled — frames sent as ADR-018-enc");
        } else {
            ESP_LOGW(TAG, "AES-GCM init failed — streaming unencrypted");
        }
    } else {
        ESP_LOGW(TAG, "No AES key in NVS — streaming unencrypted (plaintext ADR-018)");
    }
